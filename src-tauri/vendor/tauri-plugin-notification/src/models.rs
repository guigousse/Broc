...
// custom ISO-8601 serialization compatible avec le parseur natif iOS.
mod iso8601 {
    ...
    pub fn serialize<S: Serializer>(
        ...
        // Le parseur natif iOS (ios/Sources/Notification.swift) exige EXACTEMENT
        // le format `yyyy-MM-dd'T'HH:mm:ss.SSS'Z'` (3 décimales + `Z` littéral,
        // UTC). Le format ISO-8601 par défaut du crate `time` émet 9 décimales
        // et un offset `+00:00`, que ce parseur rejette → la notification n'est
        // jamais programmée (erreur silencieuse). On émet donc le format attendu,
        // converti en UTC. PATCH LOCAL (cf. [patch.crates-io] dans Cargo.toml).
        let dt = datetime.to_offset(UtcOffset::UTC);
        let s = format!(
